...
def get_taxi_data(date_str) -> List[Dict] :
	"""
...
def get_weather_data(date_str) -> List[Dict] :
	"""
...
def upload_to_s3(data: Dict, folder: str, filename: str) -> None :
	...
	"""
	if not data :
		raise ValueError(f'data for {filename} is empty')
	s3_key = f'raw_data/to_process/{folder}/{filename}'
...
def lambda_handler(event, context) :
	...
	Steps:
	1. create the date (today minus 2 months)
